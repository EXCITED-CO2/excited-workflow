...
def train_model(df: pd.DataFrame, x_keys: list[str], y_key: str) -> tuple[Any, Any]:
    ...
    model = pycs.compare_models(
        include=["lightgbm"], n_select=1, round=7, cross_validation=False
    )
...
def validate_model(
    ...
    model_vars = [
        f"{var}: {ds[var].attrs['long_name']}" if "long_name" in ds[var].attrs else var
        for var in x_keys
    ]
...
if __name__ == "__main__":
    ...
    ds_na = mask_region(regions_path, cb_file, "transcom_regions", ds_input)
